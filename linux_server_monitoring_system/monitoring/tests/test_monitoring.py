...
class MonitoringServiceTests(TestCase):
    def test_collects_and_stores_metrics(self):
        ...
        self.assertEqual(memory_used.value, Decimal("1000000000"))
        self.assertEqual(memory_used.unit, "bytes")

        memory_available = samples.get(metric_name="memory_available")
        self.assertEqual(memory_available.value, Decimal("3000000000"))
        self.assertEqual(memory_available.unit, "bytes")

        memory_usage = samples.get(metric_name="memory_usage")
        self.assertEqual(memory_usage.value, Decimal("25.000000"))
        self.assertEqual(memory_usage.unit, "percent")

        swap_usage = samples.get(metric_name="swap_usage")
        self.assertEqual(swap_usage.value, Decimal("10.000000"))
